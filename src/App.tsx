...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('hieu_tu_be_user');
    if (savedUser) {
  ...
  const handleLogin = (username: string) => {
    ...
    setUser(newUser);
    localStorage.setItem('hieu_tu_be_user', JSON.stringify(newUser));
  };

  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('hieu_tu_be_user');
  };
    ...
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      {internalVideoMessage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          ></div>
          <div className="relative bg-[#1e1e1e] border border-[#3e3e3e] rounded-3xl p-10 max-w-lg w-full text-center shadow-2xl">
             <button 
                onClick={() => setInternalVideoMessage(null)}
